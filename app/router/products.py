...
from app.db import models_product
...
def List_of_products(db:Session = Depends(get_db)):
    ...
    db_products = db.query(models_product.Product).all()

    return db_products

@router.get("/products/{id}",tags=["Operations"])
def search_product(id: int, db:Session = Depends(get_db)):
    db_product = db.query(models_product.Product).filter(models_product.Product.id == id).first()
    if not db_product:
...
def add_product(product: ProductBase, db: Session = Depends(get_db)):
    db.add(models_product.Product(**product.model_dump()))
    db.commit()
    return product
